...
After purchasing the portal, the very next step is to open it. Opening the portal occurs on [Base](/base-migration) where gamers have to sign a transaction. After the transaction is signed, the **Proof of Play VRF** is called behind-the-scenes. When the function is executed, the portal will be assigned a random number, ensuring that the Aavegotchi's traits will be provably random. (Legacy Polygon used Chainlink VRF.)
